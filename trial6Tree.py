...
class DecisionTree:
    ...
    def _linear_combination(self, X, y, feat_idxs):
        # Fit logistic regression with Ridge regularization to learn coefficients and select features
        ridge = Ridge(alpha=0.1)  # You can adjust the alpha value as needed
        ridge.fit(X[:, feat_idxs], y)
        
        # Extract the indices of non-zero coefficient features selected by Ridge
        selected_feat_idxs = np.arange(len(feat_idxs))  # All features are selected with Ridge
